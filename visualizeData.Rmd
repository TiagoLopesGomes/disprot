...
gl_activator = readr::read_tsv(paths[['gl_activator']])
gl_repressor = readr::read_tsv(paths[['gl_repressor']])
...
```{r}
g = compareGeneLists(gls = list(activator = gl_activator, repressor = gl_repressor),
                     disorderPredAll = disorderPredAll,
                     proteome = idMap,
                     features = c("AAC", "DC", "CTriad", "PAAC", "Geary"),
                     minLength = 30)
```
