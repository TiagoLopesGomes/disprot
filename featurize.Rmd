...
```{r featuresDB}
# read data
trainset_files = sort(list.files(paths[['dirDataML']], pattern = "train_set\\d+\\.tsv.gz", full.names = FALSE))
testset_files = sort(list.files(paths[['dirDataML']], pattern = "test_set\\d+\\.tsv.gz", full.names = FALSE))
...
if (createFullFeaturesDB) {
  ...
  featuresDB = createFeaturesDB(paths[['uniprotProteome']], save = paths[['featuresDB_full_100']], lambda = 100, cores = cores,
                                features = c("APAAC", "PAAC", "Geary", "Moran", "MoreauBroto", "QSO", "SOCN"))
} else {
  if (useExistingFeaturesDB && file.exists(paths[['featuresDB_full']])) {
    featuresDB = readRDS(paths[['featuresDB_full']])
  } else if (useExistingFeaturesDB && file.exists(paths[['featuresDB_small']])) {
    featuresDB = readRDS(paths[['featuresDB_small']])
  } else {
    # create small features database consisting only of unique identifiers in data
    ids = NULL
    for (i in 1:nTrainFiles) {
      trainset = readr::read_tsv(file.path(paths[['dirDataML']], trainset_files[i]))
      ids = c(ids, trainset[["p1_uniprotName"]], trainset[["p2_uniprotName"]])
    }
    for (i in 1:nTestFiles) {
      testset = readr::read_tsv(file.path(paths[['dirDataML']], testset_files[i]))
      ids = c(ids, testset[["p1_uniprotName"]], testset[["p2_uniprotName"]])
    }
    ids = sort(unique(ids))

    featuresDB = createFeaturesDB(paths[['uniprotProteome']], ids = ids, save = paths[['featuresDB_small']], cores = cores)
  }
...
labelsAll_aug = rbind(dataAll_trim[['label']], dataAll_trim[['label']])
readr::write_lines(labelsAll_aug, paths[['IDPpi_labelsAll_aug']])
rm(aug)
```
